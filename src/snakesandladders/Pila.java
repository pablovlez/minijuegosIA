...
public class Pila {
    ...
    public ArrayList verDatos() {
        ...
        if (estaVacia()) {
            ...
            int j = tope;
            for (int i = j; i >= 0; i--) {
                //arreglo.add(arg.get(i));
                arreglo.add(k + 1, "posicion " + i + " es de tipo " + arg.get(i).getTipo()+ " utilidad "+ arg.get(i).getUtilidad()+" Informacion "+ arg.get(i).getInfo()+" Operador "+arg.get(i).getOperador());
                k++;
            }
        }
        return arreglo;
